...
aic <- c()
bic <- c()
for (lambda in seq(lambdas)) {
  # Run model
  model <- glmnet(X, y, alpha = 0, lambda = lambdas[lambda], standardize = TRUE)
  # Extract coefficients and residuals (remove first row for the intercept)
  betas <- as.vector((as.matrix(coef(model))[-1, ]))
  resid <- y - (X_scaled %*% betas)
  # Compute hat-matrix and degrees of freedom
  ld <- lambdas[lambda] * diag(ncol(X_scaled))
...
plot(log(lambdas), aic, col = "orange", type = "l",
