...
ticks <- rep(ticks, 4)
cp <- c(1.01 ** (1:20), 1.02 ** (1:20))
pre_cp <- c(rep(1,10), rep(1.01**10, 10), rep(1,10), rep(1.02**10, 10))
ii <- c(rep(1,20), rep(2,20))
dates <- rep(c(rep(1,10), rep(2,10)), 2)

dt <- data.table(ukey = ii, DataDate = dates, ticktime = ticks, close = cp, vwap = cp, pre_close = pre_cp)
...
# close to next day open, TODO check if correct
dt[ticktime == 930000L, ret_co := shift(ret_pco, 1L, type="lead", fill=0), by = ukey]
...
dt[, (mret_cols) := lapply(.SD, function (x) x - beta * weighted.mean(x, capt, na.rm = TRUE)), by = .(ukey, ticktime), .SDcols=ret_cols]
dt[, (umret_cols) := lapply(.SD, demean, w=dv), by = .(ukey, ticktime, UNIVERSE), .SDcols=mret_cols]